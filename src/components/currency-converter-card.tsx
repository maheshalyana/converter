...
export function CurrencyConverterCard() {
  ...
  useEffect(() => {
    if (errorCurrencies) {
      toast({
        title: "Error",
        description: "Could not load currencies.",
        variant: "destructive",
      });
    }
    if (errorRates) {
  ...
  const calculateConversion = useCallback(() => {
    ...
    if (rate === undefined) {
      // Should not happen if API call is specific enough
      if (!isTypingFrom) setAmountFrom("");
      else setAmountTo("");
      return;
    }
    ...
    if (isTypingFrom) {
      ...
      setAmountTo((numAmountFrom * rate).toString());
    } else {
      if (isNaN(numAmountTo) || numAmountTo === 0) {
        setAmountFrom("");
        return;
      }
      setAmountFrom((numAmountTo / rate).toString());
    }
  }, [
    amountFrom,
    amountTo,
  ...
  useEffect(() => {
    if (
      ...
      // only if amountTo is not already set
      calculateConversion();
    }
  }, [
    fromCurrency,
